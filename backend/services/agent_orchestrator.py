...
"""
    ...
    async def verify_legal_accuracy(self, query: str, response: str, context: str) -> str:
        """Article Verifier: Programmatic Firewall to eradicate post-hoc hallucinations."""
        import re
        
        # 1. СТРОГИЙ ПРОГРАММНЫЙ ФИЛЬТР (Hard Refusal Rule)
        # Ищем все номера статей, разрешенные в контексте
        allowed_articles = set(re.findall(r'(?:стать[ьяеюяйнах]+|ст\.)\s*(\d+)', context, re.IGNORECASE))
        
        # Ищем все номера статей, которые ИИ попытался процитировать
        ...
        if unauthorized:
            ...
            return (
                ...
                f"Нейросеть попыталась сгенерировать ответ, сославшись на статьи (**{', '.join(unauthorized)}**), "
                f"которые **НЕ прошли проверку релевантности** или **отсутствуют** в базе знаний по данному вопросу.\n\n"
                f"Согласно жестким правилам безопасности системы, предоставление юридически неподтвержденной информации запрещено. "
                f"Надежных норм для ответа на ваш запрос не найдено."
            )

        # 2. Логическая проверка (если статьи валидны)
...
ПРАВИЛО: Если ИИ сделал ложный логический вывод, исказив суть разрешенной статьи (например, связал статью об охране труда с увольнениями), перепиши ответ, удалив эту ложную связку.
...
"""
    ...
    async def process_chat_query(self, query: str, history: List[Dict], user_role: str, user_plan: str = "freemium", total_messages: int = 0, db: Any = None, user_id: int = None, forced_lang: str = None) -> Dict[str, Any]:
        ...
        # This is the most critical anti-hallucination measure.
        # We explicitly tell the model WHAT it can cite based on what RAG actually found.
        if context.strip():
